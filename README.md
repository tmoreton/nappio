...
The media path is peer to peer whenever the network allows it. A small Cloudflare Worker coordinates pairing and WebRTC signaling, while Cloudflare TURN is used only when a direct connection is blocked. NapNear has no accounts and does not record audio or video.
...
7. Backgrounding or locking a Parent Unit leaves its current media preference unchanged, so an active video stream remains active until the Parent explicitly selects **Audio Only**.
8. Saved role sessions renew during authenticated use and expire after 30 days. Ending the Baby session revokes the room; ending or replacing a Parent session revokes only that Parent.

### What the service handles

| Data | Where it goes | Retention |
| --- | --- | --- |
| Camera, microphone, and talk-back media | Directly between phones, or encrypted through TURN | Not stored by NapNear |
...
- **Realtime media:** `react-native-webrtc` provides the native peer-to-peer WebRTC implementation.
...
- **Alerts:** sound, power, and interruption alerts are calculated locally. NapNear does not register for remote push notifications.
...
Use `npx expo run:android --device` for Android. Open NapNear on both phones, choose **Use as Baby Camera** on one, and **Monitor Baby** on the other.

## Self-host the backend

The hosted endpoint used by official NapNear builds is not a shared public backend for forks. A fork or redistributed build should deploy its own Worker and TURN key.
...
The checked-in Expo identifiers belong to the official NapNear build. Set these build-time values in your fork's `.env` before creating native or EAS builds:
...
NapNear is licensed under the [Apache License 2.0](LICENSE). Dependencies retain their own licenses. The `private` fields in the package manifests only prevent accidental publication to npm; they do not make the source proprietary.
