...
Use this worksheet when completing **App Privacy** in App Store Connect. It reflects the repository as reviewed on September 8, 2026; confirm the production Cloudflare settings before saving the answers.
...
- The pairing service temporarily stores a random pairing code, room identifier, hashed recovery credentials, single-use signaling tickets, and expiry timestamps. Pairing codes expire after five minutes and session records expire within 24 hours.
...
- Cloudflare Realtime TURN may process connection metadata and operational logs. Confirm the production account's retention and support-access settings.
...
- **Diagnostics:** Declare only if the confirmed Cloudflare production logging contains diagnostic data retained beyond what is necessary for the live request.
